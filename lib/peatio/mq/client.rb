...
module Peatio
  module MQ
    class Client
      ...
      def initialize
        Client.connect! unless MQ::Client.connection
      ...
      def publish(ex_name, type, id, event, payload)
        ...
        Peatio::Logger.logger.info { "published event to #{routing_key} " }
      end
      ...
      def subscribe(ex_name, &callback)
        ...
        @channel
          ...
          Peatio::Logger.logger.info "Subscribed to exchange #{ex_name}"
      end
    end
  end
end
