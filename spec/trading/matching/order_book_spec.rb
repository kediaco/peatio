...
describe Matching::OrderBook do
  ...
  context '#add' do
    ...
    it 'should not broadcast add event' do
      ...
      expect(AMQP::Queue).to receive(:enqueue).with(:slave_book, { action: 'add', order: order.attributes }, persistent: false).never
  ...
  context 'on_change callback provided' do
    ...
    it 'should notify add limit order' do
      order = Matching.mock_limit_order(type: :ask)
      expect(callback).to receive(:call).with('btcusd', :ask, order.price, order.volume)
      subject.add order
    end
    ...
    it 'should notify remove limit order' do
      ...
      expect(callback).to receive(:call).with('btcusd', :ask, '1.0'.to_d, o1.volume)
      expect(callback).to receive(:call).with('btcusd', :ask, '1.0'.to_d, o2.volume + o1.volume)
      expect(callback).to receive(:call).with('btcusd', :ask, '1.0'.to_d, o2.volume)
    ...
    it 'should notify fill top order with volume' do
      ...
      expect(callback).to receive(:call).with('btcusd', :ask, o1.price, '2.0'.to_d)
      expect(callback).to receive(:call).with('btcusd', :ask, o1.price, '1.5'.to_d)
      subject.add(o1)
